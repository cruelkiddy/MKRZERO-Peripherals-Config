...
///< Configuration of TC4 to trigger ADC, samplingRate in ksps. Hard coded to 1kHz
void setupMytimer(int samplingRate){
  ...
  REG_TC4_COUNT16_CC0 = 0xBB80;                   // Set the TC4 CC0 register as 48000 in match frequency mode, to achieve 1kHz sampling rate
  while (TC4->COUNT16.STATUS.bit.SYNCBUSY);       // Wait for synchronization
  REG_TC4_EVCTRL |= TC_EVCTRL_OVFEO;              // OVF event ouput enable

  while (TC4->COUNT16.STATUS.bit.SYNCBUSY);       // Wait for synchronization
  REG_TC4_CTRLA |= TC_CTRLA_PRESCALER_DIV1 |      // Set prescaler to 1, 48MHz/1 = 48000kHz
